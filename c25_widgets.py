...
class BottomWidgets:	# Add extra widgets to the bottom of the screen
  ...
  def Widgets_0x06(self, app, hardware, conf, frame, gbs, vertBox):
    ...
    class MultiplyingNumberFormatter:
      def __init__(self, fmt, factor):
        self.fmt = fmt
        self.factor = factor
      def __mod__(self, number):
        return self.fmt % (self.factor * number)
    att_slider = app.SliderBoxHH(frame, MultiplyingNumberFormatter("Att %d dB", 12), 0, -3, 0, self.OnAlexAtt, True)
    att_slider.slider.SetWindowStyleFlag(wx.SL_INVERSE)
    gbs.Add(att_slider, (start_row, self.start_col + 2), (1, 4), flag=wx.EXPAND)
    hardware.ChangeLNA(0)
    preamp_slider = app.SliderBoxHH(frame, MultiplyingNumberFormatter("PreAmp %d dB", 18), 0, 0, 2, self.OnPreAmp, True)
    gbs.Add(preamp_slider, (start_row, self.start_col + 6), (1, 4), flag=wx.EXPAND)
  ...
  def OnPreAmp(self, event):
    preamp_slider = event.GetEventObject()
    value = preamp_slider.GetValue()
    new_lna = -12
    new_dither = False
    if value >= 1:
      new_lna = 20
