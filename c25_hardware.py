...
class Hardware(BaseHardware):
  ...
  def HeartBeat(self):
    data = QS.hermes_to_pc()
    self.hermes_temperature = data[4] << 8 | data[5]
    self.hermes_fwd_power   = data[6] << 8 | data[7]
    self.hermes_rev_power   = data[8] << 8 | data[9]
    self.hermes_pa_current  = data[10] << 8 | data[11]
  ...
  def ChangeLNA(self, value):
    ...
    if self.hermes_code_version < 40:
      if value < 20:
        self.pc2hermes[2] |= 0x04			# C0 index == 0, C3[3]: LNA +32 dB disable == 1
        value = 19 - value
      else:
        self.pc2hermes[2] &= ~0x04		# C0 index == 0, C3[3]: LNA +32 dB enable == 0
        value = 51 - value
    else:
